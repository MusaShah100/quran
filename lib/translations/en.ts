export const en = {
  // Navigation
  nav: {
    home: "Home",
    quran: "Qur'an Reader",
    games: "Games",
    courses: "Courses",
    videos: "Videos",
    teachers: "Teachers",
    pricing: "Pricing",
    about: "About",
    contact: "Contact",
    updates: "Updates",
    admin: "Admin",
    teacherDashboard: "Teacher Dashboard",
    getFreeTrial: "Get Free Trial Class",
    getRegistered: "Get Registered",
  },
  // Homepage
  home: {
    hero: {
      badge: "🎉 Free 3-Day Trial Available",
      title: "Master the Qur'an with",
      titleHighlight: "Expert Teachers",
      description: "Join thousands learning Qur'an with certified teachers, interactive tools, and personalized guidance. Start your spiritual journey today.",
      startTrial: "Start Free Trial",
      viewCourses: "View Courses",
      happyStudents: "Happy Students",
      rating: "rating",
      reviews: "reviews",
    },
    trust: {
      activeStudents: "Active Students",
      expertTeachers: "Expert Teachers",
      studentRating: "Student Rating",
      support: "Support",
    },
    howItWorks: {
      badge: "How It Works",
      title: "Start Learning in 3 Simple Steps",
      description: "Begin your Qur'an learning journey with our proven methodology",
      step1: {
        title: "Choose Your Course",
        description: "Select from Tajweed, Hifz, or Tafseer courses tailored to your age and level",
      },
      step2: {
        title: "Meet Your Teacher",
        description: "Get matched with certified teachers and start your free 3-day trial",
      },
      step3: {
        title: "Master & Excel",
        description: "Practice with interactive tools and track your progress to mastery",
      },
      cta: "Start Your Free Trial Today",
    },
    features: {
      badge: "Features",
      title: "Everything You Need to Excel",
      description: "Comprehensive tools and features designed for effective Qur'an learning",
      interactiveReader: {
        title: "Interactive Qur'an Reader",
        description: "Three reading modes: Tajweed colors, Urdu translation, and English translation with authentic sources",
      },
      certifiedTeachers: {
        title: "Certified Teachers",
        description: "Learn from qualified instructors with years of experience and proven teaching methods",
      },
      flexibleScheduling: {
        title: "Flexible Scheduling",
        description: "Individual or group classes with timing that fits your schedule across all time zones",
      },
      audioIntegration: {
        title: "Audio Integration",
        description: "High-quality recitation with verse-by-verse playback from renowned Qaris",
      },
      progressTracking: {
        title: "Progress Tracking",
        description: "Monitor your learning journey with detailed progress reports and achievements",
      },
      communitySupport: {
        title: "Community Support",
        description: "Join a supportive learning community with 24/7 assistance and peer interaction",
      },
    },
    courses: {
      badge: "Our Courses",
      title: "Choose Your Learning Path",
      description: "Structured courses designed for different age groups and learning objectives",
      tajweed: {
        title: "Tajweed Courses",
        description: "Perfect pronunciation and recitation rules for all age groups",
        ages: "Ages 4-7, 8-18, 19-25, 25+",
        price: "From $8/session",
      },
      hifz: {
        title: "Hifz Program",
        description: "Complete Qur'an memorization with proven techniques",
        ages: "Individual classes",
        price: "From $20/session",
      },
      tafseer: {
        title: "Tafseer Classes",
        description: "Deep understanding of Qur'anic meanings and context",
        ages: "Individual & Group",
        price: "From $14/session",
      },
      startTrial: "Start Free Trial",
    },
    testimonials: {
      badge: "Testimonials",
      title: "Success Stories from Our Students",
      description: "Hear from students who transformed their Qur'an learning journey with us",
    },
    cta: {
      badge: "🎉 Limited Time Offer",
      title: "Start Your Free 3-Day Trial Today",
      description: "Experience the difference with certified teachers, interactive tools, and personalized learning. No payment required for your trial period.",
      startTrial: "Start Free Trial Now",
      talkToTeacher: "Talk to a Teacher",
      noCreditCard: "No Credit Card Required",
      cancelAnytime: "Cancel Anytime",
      support247: "24/7 Support",
    },
    contactCta: {
      title: "Have Questions? We're Here to Help!",
      description: "Get instant answers from our support team via WhatsApp or schedule a call with our education consultants.",
      whatsapp: "WhatsApp",
      contactUs: "Contact Us",
    },
  },
  // Footer
  footer: {
    description: "Interactive online Qur'an learning platform with word-by-word highlighting, Tajwīd colors, and educational games.",
    learning: "Learning",
    platform: "Platform",
    contact: "Contact",
    quranReader: "Qur'an Reader",
    learningGames: "Learning Games",
    studentVideos: "Student Videos",
    ourTeachers: "Our Teachers",
    aboutUs: "About Us",
    faq: "FAQ",
    getInTouch: "Get in touch →",
    copyright: "© 2024 QuranLearn. All rights reserved.",
    privacy: "Privacy",
    terms: "Terms",
    accessibility: "Accessibility",
  },
  // About Page
  about: {
    title: "About QuranLearn",
    description: "Empowering learners worldwide to connect with the Qur'an through innovative technology and traditional Islamic scholarship.",
    mission: {
      title: "Our Mission",
      description: "To make Qur'an learning accessible, engaging, and effective for Muslims of all ages through interactive technology while preserving traditional Islamic teaching methodologies.",
    },
    vision: {
      title: "Our Vision",
      description: "To become the world's leading platform for Qur'an education, creating a global community of learners connected through the sacred text.",
    },
    different: {
      title: "What Makes Us Different",
      description: "Combining cutting-edge technology with traditional Islamic scholarship",
      interactive: {
        title: "Interactive Learning",
        description: "Word-by-word highlighting with instant translations, grammatical analysis, and Tajwīd guidance",
      },
      certified: {
        title: "Certified Teachers",
        description: "Learn from qualified scholars with Ijazah certifications and years of teaching experience",
      },
      family: {
        title: "Family-Friendly",
        description: "Safe, age-appropriate content with parental controls and progress tracking for all family members",
      },
    },
    journey: {
      title: "Our Journey",
      description: "Key milestones in our mission to enhance Qur'an education",
    },
    values: {
      title: "Our Core Values",
      authenticity: {
        title: "Authenticity",
        description: "Staying true to traditional Islamic teachings and Qur'anic sciences",
      },
      accessibility: {
        title: "Accessibility",
        description: "Making quality Qur'an education available to everyone, everywhere",
      },
      community: {
        title: "Community",
        description: "Building connections between learners, teachers, and families",
      },
      excellence: {
        title: "Excellence",
        description: "Continuous improvement in our educational methods and technology",
      },
    },
    team: {
      title: "Meet Our Team",
      description: "Dedicated educators and technologists working together to serve the Muslim community",
      viewAll: "View All Teachers",
    },
    accreditation: {
      title: "Recognition & Accreditation",
      scholarlyReview: {
        title: "Scholarly Review",
        description: "Our curriculum and content are reviewed by a board of qualified Islamic scholars to ensure accuracy and authenticity.",
      },
      educationalStandards: {
        title: "Educational Standards",
        description: "We adhere to international standards for online education and child safety, with regular audits and updates.",
      },
    },
    ready: {
      title: "Ready to Begin Your Journey?",
      description: "Join thousands of students learning the Qur'an with our interactive platform. Start with our free tools or explore our comprehensive courses.",
      startReading: "Start Reading Now",
      getInTouch: "Get in Touch",
    },
  },
  // Contact Page
  contact: {
    title: "Get In Touch",
    description: "We'd love to hear from you! Whether you have questions about our platform, need technical support, or want to learn more about our courses, we're here to help.",
    register: {
      title: "Register or Contact",
      description: "Provide your name, email, phone with country code, country of residence, and select the course. You may add details and confirm accuracy.",
    },
    quickHelp: {
      title: "Quick Help",
      description: "Common questions and immediate assistance",
      technical: {
        title: "Technical Issues",
        description: "Having trouble with the platform? Check our FAQ section or contact support directly.",
      },
      course: {
        title: "Course Inquiries",
        description: "Questions about our courses, schedules, or enrollment? We're here to help.",
      },
      teacher: {
        title: "Teacher Bookings",
        description: "Want to book a trial lesson or have questions about our teachers?",
      },
      partnership: {
        title: "Partnership",
        description: "Interested in partnering with us or institutional access?",
      },
    },
    contactInfo: {
      title: "Contact Information",
      description: "Get in touch with our support team",
      email: "Email",
      phone: "Phone",
      liveChat: "Live Chat",
      available: "Available on our platform",
      automated: "24/7 automated support",
    },
    officeHours: {
      title: "Office Hours",
      mondayFriday: "Monday - Friday: 9:00 AM - 6:00 PM PST",
      saturday: "Saturday: 10:00 AM - 2:00 PM PST",
      sunday: "Sunday: Closed",
      response: "We strive to respond to all inquiries within 24 hours during business days.",
    },
    alternative: {
      title: "Alternative Support",
      whatsapp: "WhatsApp Support",
      telegram: "Telegram Channel",
      faq: "FAQ Section",
      available: "Available",
      join: "Join",
      browse: "Browse",
    },
  },
  // Pricing Page
  pricing: {
    title: "Pricing Plans",
    description: "Choose the pricing model that works best for you. We offer both fixed pricing and income-based options to make Qur'an education accessible to everyone.",
    fixedPricing: {
      title: "Fixed Pricing",
      subtitle: "Choose Your Plan",
      description: "Select a plan that fits your learning schedule. All plans include access to our interactive tools and certified teachers.",
      note: {
        title: "Flexible Payment",
        description: "You can choose any amount within the price range for your selected plan. Payment is monthly and can be adjusted based on your needs.",
      },
    },
    incomeBased: {
      title: "Income-Based",
      subtitle: "Pay What You Can Afford",
      description: "Our income-based pricing ensures that Qur'an education is accessible to everyone, regardless of financial circumstances. You pay 2.5% of your monthly family income.",
      note: {
        title: "Making Education Accessible",
        description: "We believe that financial circumstances should not be a barrier to learning the Qur'an. Our income-based pricing model ensures that everyone can access quality Islamic education.",
      },
    },
    courses: {
      title: "Available Courses",
      description: "All pricing plans apply to these courses:",
    },
    cta: {
      title: "Ready to Start Learning?",
      description: "Choose your pricing model and enroll in a course today. Our team will contact you within 24 hours to discuss class timings.",
    },
  },
  // Common
  common: {
    loading: "Loading…",
    toggleMenu: "Toggle menu",
  },
};

