...
export function ArabicWhiteboard2({ onBack }: { onBack?: () => void }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const ctxRef    = useRef<CanvasRenderingContext2D | null>(null);

  // ── All values used in event handlers are REFS (no stale closures) ──
  ...
  const [isRecording, _setIsRec]   = useState(false);
  ...
  // Sync setters — update ref AND state together
  ...
  const setRec  = (v: boolean) => { isRecordingRef.current=v;_setIsRec(v); };
  ...
  /* ── draw handlers — all use refs, zero stale closures ── */
  const onDown = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault(); saveState();
    const pos = getScaled(e);
    currentPts.current = [pos];
    isDrawingRef.current = true;
    // Start a new stroke entry immediately if recording with pen
    if (isRecordingRef.current && toolRef.current === "pen") {
      recordingStrokes.current = [...recordingStrokes.current, {
        points: [pos],
  ...
  const onMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    // Append point live into the current recording stroke
    if (isRecordingRef.current && t === "pen" && recordingStrokes.current.length > 0) {
      const strokes = recordingStrokes.current;
  ...
  const startRecording = () => {
    ...
    recordingStrokes.current=[];
    setRec(true);
  };
  const stopRecording = () => {
    setRec(false);
    if (!recordingStrokes.current.length) return;
    const id=nextId++;
    ...
    recordingBase.current=null; recordingStrokes.current=[];
  };
  ...
  const startLoop = (replay: SavedReplay) => {
    loopActive.current=false;
    if (loopRaf.current) { cancelAnimationFrame(loopRaf.current); loopRaf.current=null; }
    const strokes=replay.strokes.filter(s=>s.points.length>=2);
    ...
    const step=()=>{
      ...
      if (si===0&&pi===0) {
        ctx.globalCompositeOperation="source-over";
        if (replay.baseSnapshot&&replay.baseSnapshot.width===c.width&&replay.baseSnapshot.height===c.height) {
          try { ctx.putImageData(replay.baseSnapshot,0,0); }
          catch(_) { ctx.fillStyle=BG; ctx.fillRect(0,0,c.width,c.height); }
        } else { ctx.fillStyle=BG; ctx.fillRect(0,0,c.width,c.height); }
      }
      ...
      if (pi<stroke.points.length-1) {
        ...
        ctx.beginPath(); ctx.moveTo(stroke.points[pi].x,stroke.points[pi].y);
        ctx.lineTo(stroke.points[pi+1].x,stroke.points[pi+1].y); ctx.stroke(); pi++;
      } else { pi=0; si++; if (si>=strokes.length) { si=0; pauseFrames=60; } }
      loopRaf.current=requestAnimationFrame(step);
    };
    loopRaf.current=requestAnimationFrame(step);
  };

  const deleteReplay=(id:number)=>{ if(loopingId===id) stopLoop(); setSavedReplays(p=>p.filter(r=>r.id!==id)); };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 48px)",background:"#0a0a18",fontFamily:"'Segoe UI',Tahoma,sans-serif",userSelect:"none", borderRadius: "12px", overflow: "hidden"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d2b,#1a0533 50%,#0d1a33)",borderBottom:"1px solid #2a1a4a",padding:"8px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          <div>
            <div style={{color:"#fff",fontWeight:"800",fontSize:"16px",lineHeight:1}}>Arabic Whiteboard v2</div>
            <div style={{color:"#7c6aaa",fontSize:"10px",marginTop:"2px",letterSpacing:"1px"}}>ENHANCED INTERACTIVE BOARD</div>
          </div>
        </div>
        <div style={{display:"flex",gap:"10px",alignItems:"center",flexShrink:0}}>
          {isRecording&&<div style={{display:"flex",gap:"6px",alignItems:"center"}}><div style={{width:"7px",height:"7px",borderRadius:"50%",background:"#ef4444",boxShadow:"0 0 6px #ef4444"}}/><span style={{color:"#f87171",fontSize:"11px",fontWeight:"bold"}}>RECORDING</span></div>}
      ...
      <div style={{background:"#0e0e24",borderBottom:"1px solid #1e1e3f",display:"flex",alignItems:"center",gap:"8px",padding:"5px 12px",flexShrink:0,flexWrap:"wrap"}}>
        ...
        <button onClick={isRecording?stopRecording:startRecording}
          style={tbBtn(false,"",isRecording?{background:"#7f1d1d",color:"#fca5a5",boxShadow:"0 0 8px #ef444455"}:{background:"#14293d",color:"#38bdf8"})}>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#13132a",border:"1px solid #2d2a50",borderRadius:"14px",padding:"24px 28px",maxWidth:"400px",width:"100%",boxShadow:"0 8px 40px #00000099"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"13px"}}>
              {[
                ...
                {icon:"🔴",label:"Record",desc:"Press Record, draw with the pen, then Stop. Only pen strokes are saved."},
                {icon:"▶", label:"Replay",desc:"Animates your saved drawing on loop. Press Stop to end. Double-click card name to rename."},
              ].map(({icon,label,desc})=>(
      ...
      {savedReplays.length>0&&(
        <div style={{background:"#0d0d20",borderBottom:"1px solid #1a1a38",overflowX:"auto",flexShrink:0,scrollbarWidth:"none"}}>
          <div style={{display:"flex",gap:"8px",alignItems:"center",padding:"6px 12px",minWidth:"max-content"}}>
            ...
            {savedReplays.map(r=>(
              <div key={r.id} style={{display:"flex",alignItems:"center",gap:"5px",background:"#151530",border:`1px solid ${loopingId===r.id?"#22c55e":"#252545"}`,borderRadius:"8px",padding:"4px 8px",flexShrink:0,boxShadow:loopingId===r.id?"0 0 8px #22c55e44":""}}>
                {editingId===r.id?(
                  <input autoFocus value={editName} onChange={e=>setEditName(e.target.value)}
                    ...
                    style={{background:"#1e1e3f",border:"1px solid #7c3aed",borderRadius:"4px",color:"#fff",fontSize:"11px",width:"70px",padding:"2px 4px",outline:"none"}}/>
                ):(
                  <span onDoubleClick={()=>{setEditingId(r.id);setEditName(r.name);}} style={{color:"#94a3b8",fontSize:"11px",cursor:"text"}}>{r.name}</span>
                )}
                <span style={{color:"#334155",fontSize:"10px"}}>({r.strokes.length})</span>
                <button onClick={()=>loopingId===r.id?stopLoop():startLoop(r)}
                  style={{padding:"2px 8px",borderRadius:"5px",border:"none",cursor:"pointer",fontSize:"11px",fontWeight:"bold",background:loopingId===r.id?"#14532d":"#1a3a20",color:loopingId===r.id?"#4ade80":"#86efac"}}>
                  {loopingId===r.id?"⏸ Stop":"▶ Replay"}
                </button>
                <button onClick={()=>deleteReplay(r.id)} style={{padding:"2px 6px",borderRadius:"5px",border:"none",cursor:"pointer",fontSize:"11px",background:"#2d1515",color:"#f87171"}}>✕</button>
              </div>
            ))}
      ...
      <div style={{background:"#08080f",borderTop:"1px solid #141428",padding:"3px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
        <span style={{color:"#334155",fontSize:"11px"}}>
          ...
          {isRecording&&<span style={{color:"#f87171",marginLeft:"8px"}}>● Recording… ({recordingStrokes.current.length} strokes)</span>}
